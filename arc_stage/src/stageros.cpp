...
StageNode::mapName(const char *name, size_t robotID, Stg::Model* mod) const
{
    ...
    if ((positionmodels.size() > 1 ) || umn)
    {
        ...
        if ((found==std::string::npos) && umn)
        {
            snprintf(buf, sizeof(buf), "/arc/%s/%s", ((Stg::Ancestor *) mod)->Token(), name);
        }
        else
        {
            snprintf(buf, sizeof(buf), "/arc/robot_%u/%s", (unsigned int)robotID, name);
        }
...
std::string mapFiducialKeyToName(int key) {

    std::string result = "detector";
    if(key==1) {
        result = "robot_detector";
    } else if(key==2) {
        result = "victim_detector";
    } else if(key==3) {
        result = "marker_detector";
    } else if(key==4) {
        result = "debris_detector";
    }
    return result;
}
